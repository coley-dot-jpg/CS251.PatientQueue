...
public class Part2b {
	...
	private static void check_emergency() {
		...
		if (random.nextDouble() < Parameters.p_emergency) {
			...
			// -----
			int queueNum = random.nextInt(4);
			int patientNum = random.nextInt(Q[queueNum].size());
			Patient emergency = Q[queueNum].dequeue(patientNum);
			Q[3].push(emergency);
		}
	}
	...
	private static void check_promotion() {
		...
		for (int i = 1; i >= 0; i--) {
			for (int j = 0; j < Q[i].size(); j++) {
				Patient current = Q[i].get(j);
				// check if they have waited more than the escalation delay or if they have already been moved up and they have waiting more than 2 times the escalation delay
				if ((i + 1 == current.urgency() && current.wait_time(time) > Parameters.l12_escalation_delay) ||
						(i + 1 != current.urgency() && current.wait_time(time) > 2 * Parameters.l12_escalation_delay)) {
						Q[i + 1].push(Q[i].dequeue(j));
				}
			}
		}
	...
	private static void check_walkout() {
		...
		if (random.nextDouble() < Parameters.p_walkout) {
			...
			// -----
			//get random patient that has waited longer than l1_walkout_delay and dequeue them
			int randInd = random.nextInt(Q[0].size());
			while(Q[0].get(randInd).wait_time(time) < Parameters.l1_walkout_delay) {
				randInd = random.nextInt(Q[0].size());
			}
			Q[0].dequeue(randInd);
		}
	}
